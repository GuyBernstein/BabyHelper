    ...
    "include_details": "true or false to include details",
                ...
                if tool_type == ToolType.SLEEP_PATTERN_ANALYZER:
                    ...
                    if "metrics" in parameters:
                        ...
                        if not isinstance(parameters["metrics"], list):
                            ...
                            parameters["metrics"] = [m for m in parameters["metrics"] if m in valid_metrics]
                            # If no valid metrics, default to all
                            if not parameters["metrics"]:
                                parameters["metrics"] = list(valid_metrics)
                    else:
        ...
        if tool_type == ToolType.ACTIVITY_ANALYZER:
            ...
            fallback_params.update({
                ...
                "metrics": ["total_sleep", "night_sleep", "naps", "quality"]  # Default to all metrics
            })
