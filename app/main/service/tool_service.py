...
def _execute_tool_by_type(
    ...
    if tool.tool_type == ToolType.ACTIVITY_ANALYZER:
        ...
        return _execute_sleep_analyzer(db, baby_ids, user_id, parameters, tool.configuration)
...
def _aggregate_metrics(
    ...
    if 'naps' in requested_metrics:
        result['avg_naps_per_day'] = round(aggregated['total_naps'] / baby_count, precision_settings['normal_decimals'])
    ...
    if 'quality' in requested_metrics:
        if aggregated['babies_with_valid_scores'] > 0:
            ...
            if aggregated['babies_with_valid_scores'] == 1:
                ...
                result['sleep_quality_rating'] = aggregated['quality_ratings'][0] if aggregated['quality_ratings'] else \
                messages['no_data_rating']
            else:
...
def _create_empty_result(
    ...
    if 'quality' in requested_metrics:
        ...
        summary["calculation_method"] = calculation_method if calculation_method else messages['calculation_method_na']
...
def _execute_sleep_analyzer(
    ...
    """
    config = tool_config.get('configuration', {})
    defaults = config.get('defaults', {})
    ...
    if not successful_babies:
        return _create_empty_result(days, requested_metrics, calculation_method, config)

    # Aggregate only requested metrics
    aggregated_data = _aggregate_metrics(all_patterns, successful_babies, requested_metrics, config)
